...
```bash
# 发布为单文件 exe（含压缩）
dotnet publish src/ScreenTimeoutManager/ScreenTimeoutManager.csproj -c Release -r win-x64 --self-contained true -p:PublishSingleFile=true -p:EnableCompressionInSingleFile=true -p:IncludeNativeLibrariesForSelfExtract=true -o ../dist
```
